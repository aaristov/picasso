...
def fit_spot(spot):
    #Phasor implementation according to pSMLM
    #fft_values = _np.fft.fft2(spot)
    fft_values = fftpack.fft2(spot)
    WindowPixelSize = spot.shape[0]
    size_half = int(WindowPixelSize / 2)

    angX = _np.angle(fft_values[0,1])

    if angX>0:
        angX=angX-2*_np.pi

    PositionX =(_np.abs(angX)/(2*_np.pi/WindowPixelSize))-size_half

    angY = _np.angle(fft_values[1,0])

    if angY>0:
        angY=angY-2*_np.pi

    PositionY =(_np.abs(angY)/(2*_np.pi/WindowPixelSize))-size_half

    MagnitudeX = _np.abs(fft_values[0,1])
    MagnitudeY = _np.abs(fft_values[1,0])

    result = [PositionX, PositionY, 1, 1, 1, 1]
